...
describe('Funcinalidade: Catálogo de livros', () => {
    ...
    it.skip('Deve clicar no botão Adcionar à cesta', () => {
        cy.get(':nth-child(1) > .card > .card-body > .mt-auto > .d-grid > .btn-primary').click()
        cy.get('#cart-count').should('contain', 1)
    });

    it('Deve clicar em todos os botões Adcionar à cesta', () => {
        cy.get('.btn-primary').click({ multiple: true })
    });

    it('Deve clicar no primeiro botão Adcionar à cesta', () => {
        cy.get('.btn-primary').first().click()
    });

    it('Deve clicar no ultimo botão Adcionar à cesta', () => {
        cy.get('.btn-primary').last().click()
    });

    it('Deve clicar no terceiro botão Adcionar à cesta', () => {
